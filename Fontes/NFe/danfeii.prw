...
#DEFINE MAXVALORC  014                                                // Máximo de caracteres por linha de valores numéricos - Especifico CicloMed
...
If nTipo == 2
	RPTStatus( {|lEnd| DANFEProc(@oDanfe, @lEnd, cIDEnt, Nil, Nil, @lExistNFe, lIsLoja, 0)}, "Imprimindo DANFE..." )
	If lExistNFe
...
//Local cIndex	 := ""
//Local cChave	 := ""
//Local cFciclo    := ""	// Especifico Ciclomed
...
For nZ := 1 To nLenDet
	...
	nMaxCod := MaxCod(oDet[nX]:_Prod:_cProd:TEXT, 80)		// Especifico Ciclomed
	nMaxDes := 42 										    // Especifico Ciclomed
	...
	AADD(aAux, AllTrim(TransForm(nVUnit,TM(nVUnit,TamSX3("D2_PRCVEN")[1],2))))						// // Especifico Ciclomed - Wagner - 09/03/2016
	...
	If ( oDet[nX]:_Prod:_uTrib:TEXT <> oDet[nX]:_Prod:_uCom:TEXT )
		...
		AADD(aAuxCom, AllTrim(TransForm(nVUnitCom,TM(nVUnitCom,TamSX3("D2_PRCVEN")[1],2))))			// // Especifico Ciclomed - Wagner - 09/03/2016
	...
	nTamDescP:= AT("LOTE:",AllTrim(SubStr(NoChar(oDet[nX]:_Prod:_xProd:TEXT,lConverte),(nMaxDes+1))))	// Especifico CicloMed - 02/03/17
	//cAux     := AllTrim(SubStr(NoChar(oDet[nX]:_Prod:_xProd:TEXT,lConverte),(nMaxDes+1)))
	cAux     := AllTrim(SubStr(NoChar(oDet[nX]:_Prod:_xProd:TEXT,lConverte),(nMaxDes+nTamDescP)))		// Especifico Ciclomed - 02/03/17
	...
	While !Empty(cAux) .Or. !Empty(cAuxItem) .Or. !Empty(aAux[1]) .Or. !Empty(aAux[2]) .Or. !Empty(aAux[3]) .Or. !Empty(aAux[4]) .Or. !Empty(aAux[5]) .Or. !Empty(aAux[6])
		...
		nMaxCod := MaxCod(cAuxItem, 80)		// CicloMed
	...
	// Específico CicloMed - deixa pontilhado como falso
	lPontilhado := .F.	
...
// Especifico Ciclomed - Mostra o Estado da Filial corrente no Canhoto 03/12/2010
IF cFilAnt == "01"
	cFciclo := "PR"
Else
	cFciclo := "SP"
...
oDanfe:Say(006, 002, "RECEBEMOS DE "+NoChar(oEmitente:_xNome:Text,lConverte)+" OS PRODUTOS CONSTANTES DA NOTA FISCAL INDICADA AO LADO - "+cFciclo, oFont07:oFont)
...
oDanfe:Say(017, 102, "IDENTIFICAÇÃO E ASSINATURA DO RECEBEDOR     -     ( FAVOR CONFERIR A MERCADORIA ANTES DE ASSINAR A NFE )", oFont07N:oFont)
...
nLinCalc	:=	065
*/
oDanfe:Box(052,000,137,250)
oDanfe:Say(062,098, "Identificação do emitente",oFont12N:oFont)
nLinCalc	:=	075
...
If Type("oEmitente:_EnderEmit:_xCpl") <> "U"
	...
	// Especifico Ciclomed
	oDanfe:Say(nLinCalc,098, "Site: www.ciclomed.com.br",oFont08N:oFont)	
	nLinCalc+=10
	oDanfe:Say(nLinCalc,098, "E-Mail: ciclomed@ciclomed.com.br",oFont08N:oFont)		
Else
...
*/
oDanfe:Box(052,248,137,351)
oDanfe:Say(065,275, "DANFE",oFont18N:oFont)
...
oDanfe:Say(115,255,"N. "+StrZero(Val(oIdent:_NNf:Text),9),oFont10N:oFont)
oDanfe:Say(125,255,"SÉRIE "+SubStr(oIdent:_Serie:Text,1,3),oFont10N:oFont)
oDanfe:Say(135,255,"FOLHA "+StrZero(nFolha,2)+"/"+StrZero(nFolhas,2),oFont10N:oFont)
...
If nfolha==1
	If lMv_Logod
		...
		oDanfe:SayBitmap(055,003,cLogoD,095,093)
	Else
		// oDanfe:SayBitmap(045,001,cLogo,080,090)
		oDanfe:SayBitmap(055,003,cLogo,095,093)
	EndIF
Endif
...
//³Codigo de barra                                                         ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
/* ALTERAÇÃO DE POSICIONAMENTO - CICLOMED*/
oDanfe:Box(052,350,088,603)
oDanfe:Box(085,350,110,603)
oDanfe:Say(105,355,TransForm(SubStr(oNF:_InfNfe:_ID:Text,4),"@r 9999 9999 9999 9999 9999 9999 9999 9999 9999 9999 9999"),oFont12N:oFont)
oDanfe:Box(115,350,137,603)

If nFolha == 1
	oDanfe:Say(095,355,"CHAVE DE ACESSO DA NF-E",oFont12N:oFont)
	nFontSize := 28
	oDanfe:Code128C(082,370,SubStr(oNF:_InfNfe:_ID:Text,4), nFontSize )
EndIf
...
/* ALTERAÇÕES DE POSICIONAMENTO - CICLOMED*/
If Empty(cCodAutDPEC)
	If Empty(cChaveCont)
		oDanfe:Say(125,355,"Consulta de autenticidade no portal nacional da NF-e",oFont12:oFont)
		oDanfe:Say(135,355,"www.nfe.fazenda.gov.br/portal ou no site da SEFAZ Autorizada",oFont12:oFont)
	Endif
Endif
...
// Retorna o tamanho das colunas baseado em seu conteudo -  CicloMed
// 1 - COD. PROD
// 2 - DESCRIÇÃO DO PROD. /SERV. 
// 3 - NCM/SH 
// 4 - CST 
// 5 - CFOP 
// 6 - UN 
...
aTamCol := {55,180,35,15,20,13,35,43,54,42,33,25,27,26} 
...
For nY := 1 To nLenItens
	...
	If lflag
		If nL > nMaxItemP2
			...
			If lMv_Logod
				oDanfe:SayBitmap(002+nLinhavers,003,cLogoD,085,090)
				//oDanfe:SayBitmap(055,000,cLogoD,095,096)
			Else
				oDanfe:SayBitmap(002+nLinhavers,003,cLogo,085,090)
				//oDanfe:SayBitmap(055,000,cLogo,095,096)
			EndIf
	...
	If	nL > Iif( (nfolha-1)%2==0 .And. MV_PAR05==1,MAXITEMP3,nMaxItemP2)
		...
		If lMv_Logod
			...
			oDanfe:SayBitmap(003,003,cLogoD,085,090)
		Else
			//oDanfe:SayBitmap(000,000,cLogo,095,096)
			oDanfe:SayBitmap(003,003,cLogo,085,090)
		EndIf
